...
//
// Modified by the DoomInDynamo project (see vendor/ManagedDoom/VENDOR_NOTICE.md):
// StartDemo() and the demo-playback cases in Update() no longer let a demo lump
// recorded with a different engine version crash the whole session - see the
// comments at each change below.
//
    ...
    public sealed class OpeningSequence
    {
        ...
        public UpdateResult Update()
        {
            ...
            switch (currentStage)
            {
                ...
                case 1:
                    // [DoomInDynamo] demo is null when StartDemo() caught a
                    // version-mismatch exception for this lump - treat that as
                    // "demo already ended" instead of a NullReferenceException.
                    if (demo == null || !demo.ReadCmd(cmds))
                    {
                        nextStage = 2;
                    }
                    else
        ...
        private void StartDemo(string lump)
        {
            ...
            {
                // [DoomInDynamo] Some WADs bundle attract-mode demo lumps recorded
                // with a different engine version than this port targets (v1.9,
                // demo header byte 109 - see Demo.cs) - treat that the same as the
                // demo being empty/absent rather than crashing the session over a
                // title-screen demo nobody asked to see. The demo/game-case blocks
                // in Update() below null-check demo for exactly this reason.
                demo = null;
                game = null;
                state = OpeningSequenceState.Title;
            }
